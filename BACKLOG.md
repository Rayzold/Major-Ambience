...
- [ ] **No telemetry / error reporting.** Zero Sentry / Bugsnag / Rollbar deps in either app. The audio crash above + every future production bug exits with no record reaching anyone. **Action:** Sentry in desktop renderer + mobile, behind an opt-in "Send anonymous diagnostics" toggle in Settings. — **Desktop done 0.0.43**: `@sentry/react` wired via `lib/telemetry.ts` (opt-in, off by default, DSN read from `VITE_SENTRY_DSN` at build time; bundle stays inert without one). Toggle in Help menu. Diag buffer's tail attached as Sentry breadcrumbs in `beforeSend`. See `docs/TELEMETRY.md`. Mobile parity deferred to a separate PR (App Store telemetry-disclosure flow needs its own conversation).
